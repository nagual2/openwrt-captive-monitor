...
install_package() {
    ...
    opkg update || { echo "Error: Failed to update package lists" >&2; return 1; }
    opkg install dnsmasq curl ca-bundle || { echo "Error: Failed to install dependencies" >&2; return 1; }
    ...
    mkdir -p /usr/sbin/ /etc/init.d/ /etc/config/ /etc/uci-defaults/ /usr/share/licenses/openwrt-captive-monitor/ || 
        { echo "Error: Failed to create directories" >&2; return 1; }
    ...
    cp /tmp/openwrt-captive-monitor/package/openwrt-captive-monitor/files/usr/sbin/openwrt_captive_monitor /usr/sbin/ || 
        { echo "Error: Failed to copy openwrt_captive_monitor" >&2; return 1; }
    cp /tmp/openwrt-captive-monitor/package/openwrt-captive-monitor/files/etc/init.d/captive-monitor /etc/init.d/ || 
        { echo "Error: Failed to copy init script" >&2; return 1; }
    ...
    chmod +x /usr/sbin/openwrt_captive_monitor || { echo "Error: Failed to set executable permissions" >&2; return 1; }
    chmod +x /etc/init.d/captive-monitor || { echo "Error: Failed to set executable permissions" >&2; return 1; }
...
configure_package() {
    ...
    /etc/init.d/captive-monitor start || { echo "$(get_msg service_failed)" >&2; return 1; }
