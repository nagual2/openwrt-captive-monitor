...
## Ключевые риски
1. `resolveip -s` в BusyBox не поддерживается — определение IP портала может не сработать.
2. Диагностика целиком на ICMP → сети с блокировкой ping приводят к бесконечным перезапускам Wi‑Fi и держат captive-режим активным.
...
5. init-скрипт не убивает процесс при `stop` и не объявляет зависимости, что создаёт гонки при рестартах.
