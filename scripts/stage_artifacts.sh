...
if [ -d "$repo_root/dist/opkg" ]; then
    # ipk and feed indices with robust discovery
    # shellcheck disable=SC2046
    for f in "$repo_root"/dist/opkg/*.ipk "$repo_root"/dist/opkg/Packages "$repo_root"/dist/opkg/Packages.gz "$repo_root"/dist/opkg/SHA256SUMS "$repo_root"/dist/opkg/*.json "$repo_root"/dist/opkg/*.log "$repo_root"/dist/opkg/*.txt; do
        if [ -f "$f" ]; then
            printf '%s\n' "$f" >> "$list_file"
        fi
    done
...
for root in $search_roots; do
    if [ -d "$root" ]; then
        # Direct package/feed artifacts under this root with robust discovery
        # shellcheck disable=SC2046
        for f in "$root"/*.ipk "$root"/Packages "$root"/Packages.gz "$root"/SHA256SUMS* "$root"/*sha256sums* "$root"/*.json "$root"/*.log "$root"/*.txt; do
            if [ -f "$f" ]; then
                printf '%s\n' "$f" >> "$list_file"
            fi
        done
