# Объединённое руководство по проекту OpenWrt Captive Monitor для Cline AI
...
### Использование task_progress

**Всегда отслеживай прогресс через task_progress параметр:**
- Создавай checklist в начале задачи
- Обновляй статус по мере выполнения
- Используй формат: `- [ ]` для незавершенных, `- [x]` для завершенных

### Вводные данные для начала задачи

При получении новой задачи я работаю со следующими вводными:

1. **Требования пользователя**: Четкое описание того, что нужно сделать, включая конкретные цели, ограничения и примеры.

2. **Контекст проекта**:
   - Текущая рабочая директория (`c:\git\openwrt-captive-monitor`)
   - Структура файлов и существующий код (из environment_details)
   - Тип проекта и используемые технологии

3. **Информация об окружении**:
   - Операционная система (Windows 11)
   - Доступные CLI инструменты
   - IDE (Visual Studio Code)
   - Текущий статус git и открытые файлы

4. **Доступные инструменты**: Могу использовать операции с файлами (чтение, запись, поиск), выполнение команд, анализ кода и уточняющие вопросы по мере необходимости.

Я всегда создаю контрольный список task_progress и работаю итеративно через четко определенные шаги, используя один инструмент за раз и ожидая подтверждения после каждой операции.

### Инструменты Cline

Доступные инструменты:
- `read_file` - чтение файлов
- `write_to_file` - создание/перезапись файлов
- `replace_in_file` - точечные изменения в файлах
- `execute_command` - выполнение команд
- `search_files` - поиск по файлам (regex)
- `list_files` - список файлов в директории
- `browser_action` - взаимодействие с браузером
- `ask_followup_question` - уточняющие вопросы

### Workflow Cline

1. **Анализ задачи** - понять требования
2. **Исследование** - изучить существующий код
3. **Планирование** - создать task_progress checklist
4. **Реализация** - пошаговое выполнение
5. **Валидация** - проверка результата
6. **Завершение** - attempt_completion с summary

### Правила безопасности

- Всегда использовать `requires_approval: true` для:
  - Изменения системных настроек
  - Установка/удаление пакетов
  - Удаление файлов
  - Сетевые операции
  - Git push/merge операции

- Использовать `requires_approval: false` для:
  - Чтение файлов
  - Список директорий
  - Git status/log
  - Docker inspect/images/ps
  - Простые проверки
...
1. ✅ Создай task_progress checklist
2. ✅ Изучи релевантные файлы через read_file
3. ✅ Проверь существующую документацию
4. ✅ Задай уточняющие вопросы если нужно
5. ✅ Реализуй минимальное решение
6. ✅ Протестируй изменения
7. ✅ Обнови документацию если требуется
8. ✅ Используй attempt_completion с кратким summary
