...
| Curl | `wsl curl url` | `Invoke-WebRequest url` или `curl url` |
| JSON обработка | `wsl jq` | `jq` |
| Архивация | `wsl tar` | `tar` (Windows 10+) |
| Архивация 7z | `wsl 7z` | `7z` |
| VS Code CLI | `wsl code` | `code` |
| Kubernetes | `wsl kubectl` | `kubectl` |
...
| `wsl make` | Makefile требует Linux окружение (доступен через WSL) |
| `wsl shellcheck script.sh` | ShellCheck доступен только через WSL |
...
## Автоматизированное авто-версионирование
...
# Запуск параллельных сборок через PowerShell Jobs
foreach ($arch in $architectures) {
    ...
    Start-Job -ScriptBlock {
        ...
        docker build --build-arg SDK_TARGET=$target --build-arg SDK_SUBTARGET=$subtarget -t sdk:$target-$subtarget .
    } -ArgumentList $target, $subtarget
}
...
# Выполнить команду
ssh root@192.168.35.127 "uname -a"
...
```powershell
# Распаковка tar архива (нативный tar в Windows 10+)
...
gh pr list --json number,title,state | jq '.[] | select(.state == "OPEN") | {number: .number, title: .title}'

# Извлечение значений (raw output)
gh run list --limit 1 --json databaseId | jq -r '.[0].databaseId'

# Преобразование JSON
jq '.version = "1.0.0"' package.json > package-new.json
```
...
# Работа с JSON через jq (нативный инструмент)
gh run list --limit 5 --json databaseId,status,conclusion,name | jq '.[] | {id: .databaseId, status: .status, name: .name}'

# Отмена старых запусков (используем нативный jq)
gh run list --status in_progress --branch $(git branch --show-current) --json databaseId | jq -r '.[].databaseId' | ForEach-Object { gh run cancel $_ }

# Или без jq (чистый PowerShell)
...
# GitHub Actions проблемы (используем нативный jq)
gh run list --status failure --limit 1 --json databaseId | jq -r '.[0].databaseId' | ForEach-Object { gh run view $_ --log > failed_job_log.txt }
...
### Нативные Windows инструменты (используй напрямую)

**Основные инструменты разработки:**
- `git` - система контроля версий
- `gh` - GitHub CLI
- `docker` - контейнеризация
- `python` - интерпретатор Python (версия 3.x)
- `pip` - менеджер пакетов Python
- `curl` - HTTP клиент
- `ssh` / `scp` - SSH клиент и копирование файлов
- `act` - локальное тестирование GitHub Actions
- `yamllint` - валидатор YAML
- `winget` - менеджер пакетов Windows

**Дополнительные инструменты:**
- `jq` - обработка JSON (используется с `gh run list --json ... | jq`)
- `kubectl` - Kubernetes CLI (входит в Docker Desktop)
- `7z` - архиватор 7-Zip
- `tar` - архиватор (встроен в Windows 10+)
- `code` - VS Code CLI (открытие файлов/папок из командной строки)
- `notepad` - текстовый редактор Windows

### Инструменты доступные через WSL

**Инструменты для работы с bash скриптами:**
- `wsl shellcheck` - статический анализатор bash скриптов
- `wsl shfmt` - форматтер bash скриптов
- `wsl make` - система сборки (Makefile)
- `wsl python3` - Python 3 (если нужна именно команда python3)
- `wsl npx` - Node.js package runner (если нужен Node.js)
...
# Форматирование bash скрипта
wsl shfmt -i 2 -ci -sr -w openwrt_captive_monitor.sh

# Сборка через Makefile
wsl make -C package/openwrt-captive-monitor

# Использование python3 (если нужна именно эта команда)
wsl python3 script.py

# Использование npx (если нужен Node.js)
wsl npx some-package
```
...
**OpenSSH for Windows 10.0p2** - нативный SSH клиент Windows
...
### Преимущества нативного OpenSSH

- ✅ Не требует WSL
- ✅ Быстрее чем через WSL (нет overhead)
- ✅ Интегрирован с Windows
- ✅ Работает с Windows SSH Agent
- ✅ Поддерживает все стандартные опции OpenSSH

### Использование в проекте

**Подключение к OpenWrt тестовому окружению:**
```powershell
# Прямое подключение (без WSL)
ssh root@192.168.35.127

# Выполнить команду
ssh root@192.168.35.127 "uname -a"

# Копировать пакет на роутер
scp dist/openwrt-captive-monitor_*.ipk root@192.168.35.127:/tmp/

# Копировать с роутера
scp root@192.168.35.127:/tmp/file.txt ./
```
...
**Используй нативный SSH (предпочтительно):**
- ✅ Для обычных SSH подключений
- ✅ Для SCP копирования файлов
- ✅ Для выполнения команд на удаленном хосте
- ✅ Для работы с SSH ключами

**Используй WSL SSH только когда:**
- ⚠️ Нужны специфичные для Linux SSH features
- ⚠️ Скрипт уже написан для bash и использует SSH
- ⚠️ Требуется совместимость с Linux SSH config
...
wsl scp package.ipk root@192.168.35.127:/tmp/
```
...
**Экономия:** ~100-200ms на каждый вызов SSH

## Curl на Windows

### Установленная версия

**curl 8.16.0 (Windows)** - нативный HTTP клиент Windows

**Поддерживаемые протоколы:**
- HTTP/HTTPS, FTP/FTPS, IMAP/IMAPS, POP3/POP3S
- SMTP/SMTPS, LDAP/LDAPS, WebSocket (ws/wss)
- MQTT, Telnet, TFTP, IPFS/IPNS

**Возможности:**
- AsynchDNS, HSTS, HTTPS-proxy
- IPv6, Kerberos, NTLM, SPNEGO
- SSL/TLS (Schannel), zlib compression
- Unicode, UnixSockets

### Использование в проекте

**Проверка доступности OpenWrt SDK:**
```powershell
# Проверить HTTP статус
curl -I https://downloads.openwrt.org/releases/23.05.5/targets/x86/64/

# Скачать sha256sums
curl -fsSL https://downloads.openwrt.org/releases/23.05.5/targets/x86/64/sha256sums

# Скачать файл с прогрессом
curl -L -o sdk.tar.xz https://downloads.openwrt.org/releases/23.05.5/targets/x86/64/openwrt-sdk-23.05.5-x86-64_gcc-12.3.0_musl.Linux-x86_64.tar.xz
```

### Рекомендуемые опции для надежной загрузки
...
curl `
  ...
  --max-time 3600 `         # Maximum time (1 hour)
  --connect-timeout 60 `     # Connection timeout (60s)
  --speed-limit 1000 `      # Minimum speed (1KB/s)
  --speed-time 30 `         # Time to maintain speed (30s)
  -C - `                    # Continue interrupted download
  -o output.tar.xz `        # Output file
  $url
```
...
$statusCode = curl -s -o $null -w "%{http_code}" $url

if ($statusCode -eq 200) {
    Write-Host "✅ URL доступен" -ForegroundColor Green
} elseif ($statusCode -eq 404) {
    Write-Host "❌ Файл не найден (404)" -ForegroundColor Red
} else {
    Write-Host "⚠️  HTTP $statusCode" -ForegroundColor Yellow
}
```

### Когда использовать curl vs Invoke-WebRequest

**Используй curl (предпочтительно):**
- ✅ Для загрузки файлов
- ✅ Для проверки HTTP статусов
- ✅ Для работы с REST API
- ✅ Когда нужны продвинутые опции (retry, resume)
- ✅ Для совместимости с bash скриптами

**Используй Invoke-WebRequest когда:**
- ⚠️ Нужна глубокая интеграция с PowerShell
- ⚠️ Требуется парсинг HTML
- ⚠️ Нужны PowerShell объекты вместо raw данных

### Примеры оптимизации

**❌ Неоптимально (через WSL):**
```powershell
wsl curl -fsSL https://example.com/file.tar.gz -o file.tar.gz
```
...
**Экономия:** ~100-200ms на каждый вызов curl
...
Файл `.cursor/settings.json` содержит настройки интерфейса и проекта:
- Расположение боковых панелей (файлы слева, чат справа)
- Настройки терминала (PowerShell Core по умолчанию)
- Настройки SSH для Remote-SSH
- Настройки форматирования кода

### Использование Cursor Chat

- Чат доступен через вторичную боковую панель справа
- Используй `Ctrl+L` для быстрого доступа к чату
- Команды выполняются через встроенный терминал

### Интеграция с проектом

- Cursor автоматически использует настройки из `.cursor/settings.json`
- Remote-SSH настроен для подключения к OpenWrt тестовому окружению
- Терминал по умолчанию - PowerShell Core для нативных Windows команд
