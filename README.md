...
For each supported matrix target (`ath79-generic`, `ramips-mt7621`) it fetches the
matching OpenWrt SDK, performs `feeds update/install`, runs `defconfig`, and
publishes the resulting `.ipk` together with `Packages_<target>` indexes ready for
feed hosting or releases.
