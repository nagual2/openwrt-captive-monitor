...
**openwrt-captive-monitor** - это легковесный OpenWrt помощник, который мониторит WAN подключение, обнаруживает captive portals и временно перехватывает LAN DNS/HTTP трафик, чтобы клиенты могли пройти аутентификацию. После восстановления интернет-доступа помощник автоматически очищает dnsmasq overrides, HTTP redirects и NAT rules.
...
**Основная активность разработки - исправление проблем совместимости с BusyBox ash и ShellCheck**
...
1. **df81fc4** (25 окт 2025) - **fix(shellcheck): clean up excessive disable annotations**
   ...
   - Очищено форматирование кода и удалены избыточные предупреждения
...
3. **0616788** (25 окт 2025) - **fix(shellcheck): comprehensive ash compatibility fixes**
   ...
   - Исправлена логика обнаружения workflow файлов с POSIX-совместимыми командами
...
4. **0f7a7bb** (25 окт 2025) - **fix(shellcheck): add comprehensive ash compatibility annotations**
   - Добавлено отключение SC2153 для использования переменных окружения
...
8. **d9ae7a4** (25 окт 2025) - **docs(build): add Windows build instructions**
   ...
   - Документированы GitHub Actions как основной метод для Windows пользователей
...
10. **fe78ae9** (25 окт 2025) - **chore(repo): enforce trunk protection, improve CI hygiene, and document merge rules**
    - Обновлены триггеры workflow для main/PR и всех разрешенных префиксов веток
...
19. **97bfac3** (22 окт 2025) - **feat(captive): add nftables + IPv6 support with idempotence**
    ...
    - Идемпотентная установка/очистка intercept с использованием comments/tags
...
Проект `openwrt-captive-monitor` демонстрирует высокую активность разработки с фокусом на качество кода, совместимость и автоматизацию. Основные усилия были направлены на:
...
Проект готов к production использованию и дальнейшему развитию с четко установленными процессами разработки.
