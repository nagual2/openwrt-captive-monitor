...
The GitHub Actions workflow **Build OpenWrt packages** produces release-ready builds
for each supported target (`ath79-generic`, `ramips-mt7621`). It pulls the
...
- `Packages_<target>.gz` (with the plain `Packages` and `Packages.manifest` variants)
