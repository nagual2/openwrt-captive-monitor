...
- ✅ Проверяет доступность интернета через ping и контрольные
HTTP-запросы
- 🛰️ Определяет наличие captive-портала по редиректам/HTML ответам и
извлекает URL авторизации
- 🌐 Временно перенаправляет DNS-запросы клиентов LAN на IP роутера
(dnsmasq add-on)
- 📡 Запускает легкий busybox httpd с мгновенным редиректом на страницу
портала
- 🔀 Создает NAT-правила для HTTP-трафика (порт 80) клиентов LAN → httpd:8080
с автоопределением backend (iptables/ip6tables или nftables)
- 🌍 Поддерживает IPv6: перенаправление DNS/HTTP для клиентов при
наличии адреса на LAN-интерфейсе (ip6tables/nftables)
- 🔒 HTTPS-трафик не перехватывается (HSTS не ломается, требуется
открыть HTTP-сайт)
- ⏳ Автоматически отслеживает восстановление интернета и
выполняет очистку
- ✨ Останавливает перехват, удаляет DNS-конфиг и firewall-правила
после авторизации
...
- `option lan_interface` — вручную указать LAN-интерфейс, если
автоопределение некорректно.
- `option lan_ip`/`option lan_ipv6` — зафиксировать адреса, если они
нестандартные или недоступны до запуска службы.
- `option firewall_backend` — принудительно выбрать `iptables`/`nftables` (по
умолчанию определяется автоматически).

> Значения в примере по умолчанию остаются пустыми, поэтому
автоопределение продолжит работать до тех пор, пока вы не
зададите конкретные параметры.
...
Скрипт читает переменные окружения при запуске, поэтому можно
переопределять значения по умолчанию без редактирования файла:
...
export PING_SERVERS="1.1.1.1 9.9.9.9"         # Список адресов для ping (оставьте пустым,
чтобы пропустить)
...
> Поддерживаются также переменные для таймаутов (`PING_TIMEOUT`,
`CAPTIVE_CURL_TIMEOUT`, `DNS_LOOKUP_TIMEOUT` и др.). Любые значения, заданные через
окружение, используются, если переменная не пуста.
...
Секция `КОНФИГУРАЦИЯ` теперь разделена на значения по умолчанию
(`DEFAULT_*`) и присваивания, которые можно переопределить через
окружение:
...
> Если переменная окружения задана пустой строкой (например
`INTERNET_HTTP_PROBES=`), соответствующая проверка будет пропущена.
...
## Большинство доменов должны возвращаться с IP LAN роутера; домен
портала — с реальным IP
```
...
- Проверено на OpenWrt 21.02 (iptables/fw3). Для корректной работы требуется
наличие iptables-legacy.
- На OpenWrt 22.03+ (fw4/nftables) правила iptables могут не применяться. Нужно
включить совместимость с iptables-legacy или перенести правила в nftables
