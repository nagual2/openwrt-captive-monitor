...
> Полевой лаборатории нет — опираемся на статические проверки,
эмуляцию OpenWrt
...
- `sh -n package/openwrt-captive-monitor/files/etc/init.d/captive-monitor` — проверка синтаксиса
init-скрипта
...
2. **Сценарии**:
- `openwrt_captive_monitor --oneshot` при рабочем интернете — убеждаемся, что
перехват не активируется, правила не остаются.
     - **WAN down**:
    - Отключить интернет
...
- Подключиться к сети с авторизацией (например, публичный
хот-спот)
    - Проверить редирект
...
- Повторить локальные и эмуляционные проверки после каждой
правки:
  - Firewall
  - Health-check
  - Init-скрипты
- Пройти чек-лист `docs/RELEASE_CHECKLIST.md` (версия, changelog, контроль
зависимостей).
- Проверить:
...
- Провести smoke-тест `scripts/build_ipk.sh` в CI и вручную удостовериться, что
пакет корректно устанавливается и удаляется (`opkg remove`).
